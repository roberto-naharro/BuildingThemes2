...
where slightly smaller models can still be selected and the building is stretched in depth
to fill the lot (the extra rows become its yard) — enable the **Allow smaller building
models to occupy larger lots** option in District Options. Note that only depth is filled
this way: a narrower model in a wider lot still leaves a strip along the side.
...
| Allow smaller building models to occupy larger lots (vanilla footprint) | When enabled, slightly smaller models can still be selected and the building is stretched in depth to fill the lot — matching original game behavior (only depth is filled; a narrower model still leaves a side strip). When disabled (default), the plot shrinks to the model's actual size, leaving remaining space for other buildings. See [How Spawning Works](#how-spawning-works). |
